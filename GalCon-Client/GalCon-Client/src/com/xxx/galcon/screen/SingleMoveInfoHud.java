...
public class SingleMoveInfoHud extends Group {
	...
	public SingleMoveInfoHud(Move move, AssetManager assetManager, ShaderProgram fontShader, UISkin skin, float width,
			float height) {
		this.move = move;
	...
	private void createLabels() {
		ShaderLabel numberOfShips = new ShaderLabel(fontShader, "Sending " + move.shipsToMove, skin,
				Constants.UI.DEFAULT_FONT);
		numberOfShips.setX(getX());
		addActor(numberOfShips);

		ShaderLabel duration = new ShaderLabel(fontShader, "Duration " + Math.round(move.duration), skin,
				Constants.UI.DEFAULT_FONT);
