...
public class ShaderTextField extends Widget implements Disableable {
	...
	ShaderTextFieldStyle style;
	String text, messageText;
	private CharSequence displayText;
	int cursor;
	private Clipboard clipboard;
	ShaderTextFieldListener listener;
	ShaderTextFieldFilter filter;
	...
	private void initialize() {
		addListener(inputListener = new ClickListener() {
			...
			public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
				...
				if (stage != null)
					stage.setKeyboardFocus(ShaderTextField.this);
				keyboard.show(true);
				return true;
			}
			...
			public boolean keyDown(InputEvent event, int keycode) {
				...
				Stage stage = getStage();
				if (stage != null && stage.getKeyboardFocus() == ShaderTextField.this) {
				...
				if (stage != null && stage.getKeyboardFocus() == ShaderTextField.this) {
					if (character == BACKSPACE) {
						...
						if (character != ENTER_DESKTOP && character != ENTER_ANDROID) {
							if (filter != null && !filter.acceptChar(ShaderTextField.this, character))
								return true;
						}
					...
					if (listener != null)
						listener.keyTyped(ShaderTextField.this, character);
					return true;
				} else
					return false;
			}
	...
	public void setStyle(ShaderTextFieldStyle style) {
		if (style == null)
	 ...
	 * present in the {@link BitmapFont}
	 */
	 ...
	 */
	public ShaderTextFieldStyle getStyle() {
		return style;
	}
	...
	/**
	 * Copies the contents of this ShaderTextField to the {@link Clipboard}
	 * implementation set on this ShaderTextField.
	 */
	public void copy() {
	...
	/**
	 * Copies the selected contents of this ShaderTextField to the
	 * {@link Clipboard} implementation set on this ShaderTextField, then
	 * removes it.
	 */
	public void cut() {
		if (hasSelection && !passwordMode) {
	 ...
	 * ShaderTextField to this ShaderTextField.
	 */
	void paste() {
		...
		if (content != null) {
			...
			for (int i = 0; i < content.length(); i++) {
				if (maxLength > 0 && text.length() + buffer.length() + 1 > maxLength)
					break;
	...
	/**
	 * Focuses the next ShaderTextField. If none is found, the keyboard is
	 * hidden. Does nothing if the text field is not in a stage.
	 * 
	 * @param up
	 *            If true, the ShaderTextField with the same or next smallest y
	...
	public void next(boolean up) {
		...
		ShaderTextField textField = findNextTextField(stage.getActors(), null, tmp2, tmp1, up);
	...
	private ShaderTextField findNextTextField(Array<Actor> actors, ShaderTextField best, Vector2 bestCoords,
			Vector2 currentCoords, boolean up) {
		for (int i = 0, n = actors.size; i < n; i++) {
			Actor actor = actors.get(i);
			if (actor == this)
				continue;
			if (actor instanceof ShaderTextField) {
				ShaderTextField textField = (ShaderTextField) actor;
	 ...
	 *            May be null.
	 */
	public void setTextFieldListener(ShaderTextFieldListener listener) {
	 ...
	 *            May be null.
	 */
	public void setTextFieldFilter(ShaderTextFieldFilter filter) {
		this.filter = filter;
	}
	...
	public ShaderTextFieldFilter getTextFieldFilter() {
		return filter;
	}
	 ...
	 * @author mzechner
	 */
	static public interface ShaderTextFieldListener {
		public void keyTyped(ShaderTextField ShaderTextField, char key);
	}
	 ...
	 * @author mzechner
	 */
	static public interface ShaderTextFieldFilter {
		/**
		 * @param ShaderTextField
		 * @param key
		 * @return whether to accept the character
		 */
		public boolean acceptChar(ShaderTextField ShaderTextField, char key);

		static public class DigitsOnlyFilter implements ShaderTextFieldFilter {
			@Override
			public boolean acceptChar(ShaderTextField ShaderTextField, char key) {
	...
	/**
	 * The default {@link OnscreenKeyboard} used by all {@link ShaderTextField}
	...
	/**
	 * The style for a text field, see {@link ShaderTextField}.
	 * 
	 * @author mzechner
	 * @author Nathan Sweet
	 */
	static public class ShaderTextFieldStyle {
		...
		public ShaderTextFieldStyle() {
		}

		public ShaderTextFieldStyle(BitmapFont font, Color fontColor, Drawable cursor, Drawable selection,
				Drawable background) {
		...
		public ShaderTextFieldStyle(ShaderTextFieldStyle style) {
