...
public class LevelUpOverlay extends Overlay {
	...
	private void createLevelLabel() {
		ShaderLabel levelLabel = new ShaderLabel(resources.fontShader, "Level "
				+ ConfigResolver.getRankForXp(player.xp).level, resources.skin, Constants.UI.LARGE_FONT, Color.YELLOW);
		levelLabel.setX(0);
	...
	private void createReachLabel() {
		reachedLabel = new ShaderLabel(resources.fontShader, "You've Reached", resources.skin,
	...
	private void createMapUnlock() {
		...
		if (unlockedMap != null) {
			...
			TextureRegionDrawable mapTex = new TextureRegionDrawable(resources.levelAtlas.findRegion(""
					+ unlockedMap.key));
			...
			ShaderLabel unlockLabel = new ShaderLabel(resources.fontShader, "New Map Unlocked!", resources.skin,
					Constants.UI.DEFAULT_FONT, Color.WHITE);
			unlockLabel.setX(0);
			...
			ShaderLabel desc = new ShaderLabel(resources.fontShader, unlockedMap.description, resources.skin,
			...
			ShaderLabel title = new ShaderLabel(resources.fontShader, unlockedMap.title, resources.skin,
					Constants.UI.X_SMALL_FONT, Color.BLACK);
