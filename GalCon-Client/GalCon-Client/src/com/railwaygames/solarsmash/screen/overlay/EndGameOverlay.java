...
public abstract class EndGameOverlay extends Overlay {
	...
	private List<Tip> tips = new ArrayList<Tip>() {
		{
			...
			add(new Tip(0L,
					"Use the number of rounds a ship takes to move to time fleets from multiple planets to attack together."));
			add(new Tip(0L,
					"Larger planets have a higher ship build rate. A higher overall build rate will help you overrun the enemy"));
			add(new Tip(2L,
					"Planets with moons convey special abilities but have a build rate of 1. Capture them wisely"));
			add(new MapSpecificTip(3L, "Increased speed enables your ships to move from planet to planet in less turns"));
			add(new MapSpecificTip(6L,
					"Harvest an ability planets moon to gain an increase in power. This will destory the moon after a number of rounds"));
			add(new MapSpecificTip(6L,
					"Capturing a planet which is under harvest from the enemy will give you a ship bonus."));
	...
	private UIConnectionResultCallback<GameQueue> gameQueueCallback = new UIConnectionResultCallback<GameQueue>() {
		...
		public void onConnectionResult(GameQueue result) {
			...
			if (invites.isEmpty() && GameLoop.getUser().coins > 0) {
				if (scrollList != null) {
					scrollList.remove();
				}
				createRematchButton();
	...
	private Player opponent() {
		return gameBoard.allPlayersExcept(GameLoop.getUser().handle).get(0);
	}
	...
	private ClickListener rematchClickListener = new ClickListener() {
		public void clicked(com.badlogic.gdx.scenes.scene2d.InputEvent event, float x, float y) {
			...
			UIConnectionWrapper.invitePlayerForGame(new UIConnectionResultCallback<GameBoard>() {
				...
				public void onConnectionError(String msg) {
					Overlay errorOverlay = new DismissableOverlay(resources, new TextOverlay(msg, resources),
							new ClickListener() {
								public void clicked(InputEvent event, float x, float y) {
			...
			}, GameLoop.getUser().handle, gameBoard.allPlayersExcept(GameLoop.getUser().handle).get(0).handle,
					gameBoard.map);
		};
	};
	...
	private void createTipLabel() {
		tipLabel = new ShaderLabel(resources.fontShader, "Tip: " + getTip(gameBoard.map).tip, resources.skin,
				Constants.UI.SMALL_FONT, Color.WHITE);
		tipLabel.setBounds(Gdx.graphics.getWidth() * 0.05f, Gdx.graphics.getHeight() * 0.15f,
				Gdx.graphics.getWidth() * 0.9f, tipLabel.getTextBounds().height);
