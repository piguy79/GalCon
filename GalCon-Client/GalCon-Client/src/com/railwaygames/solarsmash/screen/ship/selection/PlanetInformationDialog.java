...
import com.railwaygames.solarsmash.model.display.AbilityDisplay;
...
public class PlanetInformationDialog extends OKCancelDialog {
		...
		if (!isHarvestAvailable()) {
			...
			okButton.addListener(new ClickListener() {
				...
				public void clicked(InputEvent event, float x, float y) {
					...
					if (isHarvestAvailable()) {
						fire(new HarvestEvent(PlanetInformationDialog.this.planet));
						hide();
					}
				}
			});
...
//				Constants.UI.DEFAULT_FONT);
		...
		positionAndPlaceActor(regenName, new Point(initialPadX, planetImage.getY()
				- (regenName.getTextBounds().height * 2)));
