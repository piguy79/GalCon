...
public class GPPSignIn extends NSObject {
	...
	// @property(nonatomic, strong, readonly) GTMOAuth2Authentication
	// *authentication;
	// TODO: implement
	...
	// The Google user ID. It is only available if |shouldFetchGoogleUserID| is
	// set
	...
	// The Google user's email. It is only available if
	// |shouldFetchGoogleUserEmail|
	...
	// The Google+ user profile. It is only available if
	// |shouldFetchGooglePlusUser|
	// is set and either |trySilentAuthentication| or |authenticate| has been
	// completed successfully.
	// @property(nonatomic, strong, readonly) GTLPlusPerson *googlePlusUser;
	// TODO: implement
	/*
	 * private static final Selector googlePlusUser$ =
	 * Selector.register("googlePlusUser:");
	 * 
	 * @Bridge private native static GTLPlusPerson
	 * objc_getGooglePlusUser(GPPSignIn __self__, Selector __cmd__); public
	 * GTLPlusPerson getGooglePlusUser(){ return objc_getGooglePlusUser(this,
	 * googlePlusUser$); }
	 */
	...
	// If |attemptSSO| is true, the sign-in button tries to authenticate with
	// the
	// Google+ application if it is installed. If false, it always uses Google+
	// via
	...
	// Whether or not to fetch user email after signing in. The email is saved
	// in
	// the |GTMOAuth2Authentication| object. Note that using this flag
	// automatically
	// adds "https://www.googleapis.com/auth/userinfo.email" scope to the
	// request.
	...
	// flag automatically adds
	// "https://www.googleapis.com/auth/userinfo.profile"
	// scope to the request if a scope that provides user ID is not already
	// present.
	...
	// Returns |YES| and calls the delegate if the user has either currently
	// signed
	// in or has previous authentication saved in keychain.
	// Note that if the previous authentication was revoked by the user, this
	// method
	...
	// Starts the authentication process. Set |attemptSSO| to try single
	// sign-on.
	...
	// - (BOOL)handleURL:(NSURL *)url sourceApplication:(NSString
	// *)sourceApplication annotation:(id)annotation;
	private static final Selector handleURL$ = Selector.register("handleURL:sourceApplication:annotation:");

	@Bridge
	private native static boolean objc_handleURL(GPPSignIn __self__, Selector __cmd__, NSURL url,
			NSString sourceApplication, NSObject annotation);
	...
	// If the operation succeeds, the OAuth 2.0 token is also removed from
	// keychain.
	// The token is needed to disconnect so do not call |signOut| if
	// |disconnect| is
	// to be called.
	// - (void)disconnect;
	...
	// Gets a service object authenticated as the current user. The service
	// object
	// can execute queries, for example, to get list of people that is visible
	// to
	...
	@Bridge
	private native static GTLServicePlus objc_plusService(GPPSignIn __self__, Selector __cmd__);
