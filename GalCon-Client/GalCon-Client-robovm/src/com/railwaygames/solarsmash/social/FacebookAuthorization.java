...
public class FacebookAuthorization extends UIApplicationDelegateAdapter implements Authorizer {
		...
		if(!facebook.isLoggedIn() || !FBSession.getActiveSession().isOpen()){
			...
			facebook.login(new FacebookLoginListener() {
				...
				public void onSuccess(GraphObject user) {
					...
					GameLoop.USER.addAuthProvider(Constants.Auth.SOCIAL_AUTH_PROVIDER_FACEBOOK, user.getString("id").toString());
					...
					prefs.flush();
					listener.onSignInSucceeded(Constants.Auth.SOCIAL_AUTH_PROVIDER_FACEBOOK, FBSession.getActiveSession().getAccessTokenData().getAccessToken());
				}
	...
	public void postToFriend(final FriendPostListener listener,final String id) {
		signIn(new AuthenticationListener() {
			...
			public void onSignInSucceeded(String authProvider, String token) {
				...
				facebook.request(CommonFacebookRequests.publishFeedToFriend(id, "Solar Smash", "Download Solar Smash now.", "", "Hey, come play me in Solar Smash. Invite me using the handle \"" + GameLoop.USER.handle + "\"", "http://www.railwaygames.mobi/assets/images/logo/android_icon.png", "http://www.railwaygames.mobi", FBSession.getActiveSession().getAccessTokenData().getAccessToken(), true, new FacebookRequestListener() {
	...
	private void setupManager(){
		if(facebook == null){
			...
			FBPermission[] fbPermissions = new FBPermission[] {FBPermission.BASIC_INFO};
			...
			FacebookConfiguration fbConfiguration = new FacebookConfiguration.Builder().setAppId(APP_ID).setNamespace(APP_NAMESPACE)
				...
				.setLoginBehavior(FBSessionLoginBehavior.ForcingWebView).build();
