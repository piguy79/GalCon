...
public class FacebookManager {
	...
	public void login(final FacebookLoginListener listener) {
		// TODO change to use method session.open(FBSessionLoginBehaviour) and
		// if successful then request for additional read and
		...
		NSOperationQueue.getMainQueue().addOperation(new Runnable() {
			...
			public void run() {
				...
				if (session == null || !session.isOpen()
						|| !containsRequiredPermissions(configuration.getReadPermissions())
											...
											session.requestNewPublishPermissions(configuration.getPublishPermissions(),
													...
													new FBSessionRequestPermissionResultHandler() {
														...
														public void invoke(final FBSession session, final NSError error) {
															NSOperationQueue.getCurrentQueue().addOperation(
																	new Runnable() {
																		@Override
																		public void run() {
																			if (error != null) {
																				if (FBErrorUtility
																						.getErrorCategory(error) == FBErrorCategory.UserCancelled) {
																					listener.onCancel();
																				} else {
																				...
																				if (!containsRequiredPermissions(configuration
																					...
																					FBRequest.requestForMe().start(
																							new FBRequestHandler() {
																								@Override
																								public void invoke(
	...
	/**
	 * Makes the specified request(s) on the Facebook api. If multiple requests
	 * are specified, a batch request will be made (only works for requests
	 * without dialog ui).
	 * 
	 * @param requests
	 */
					...
					if (requests[0].isUseDialog()) {
						for (final FacebookRequest request : requests) {
							if (request.getApi().contains("feed")) {
								Foundation.log("FB session" + session);
								FBWebDialogs.presentFeedDialog(session, request.getParameters(),
										new FBWebDialogHandler() {
											@Override
											public void invoke(final FBWebDialogResult result, final NSURL url,
													final NSError error) {
												Foundation.log("FB result: " + result);
												Foundation.log("FB url: " + url);
												Foundation.log("FB Error: " + error);
												NSOperationQueue.getCurrentQueue().addOperation(new Runnable() {
													@Override
													public void run() {
														if (error != null) {
								...
								FBWebDialogs.presentRequestDialog(session, request.getParameters().get("message"),
										request.getParameters().get("title"), request.getParameters(),
										new FBWebDialogHandler() {
											@Override
											public void invoke(final FBWebDialogResult result, final NSURL url,
													final NSError error) {
						...
						for (final FacebookRequest requestOrder : requests) {
							FBRequest request = new FBRequest(session, requestOrder.getApi(), requestOrder
									.getParameters(), requestOrder.getHttpMethod());
